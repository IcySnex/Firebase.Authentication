...
using Firebase.Authentication.Sample.WinUI.Helpers;
...
public class Navigation
{
        ...
        StringComparison comparision = StringComparison.InvariantCultureIgnoreCase)
    {
        try
        {
            ...
            return items.Find(item => searchFor.Equals(searchForContent ? $"Firebase.Authentication.Sample.WinUI.Views.{item.Content}View" : item.Content.ToString(), comparision));
        }
        catch { return null; }
    }

    /// <summary>
    /// Sets a navigation view items visibility
    /// </summary>
    /// <param name="index">The string to search for</param>
    /// <param name="isVisible">The boolean wether it should be visibile</param>
    /// <returns>A boolean wether the navigation view items visibility has been set successfully</returns>
    public bool SetNavigationViewItemVisibility(
        int index,
        bool isVisible)
    {
        try
        {
            NavigationViewItem item = items.ElementAt(index);
            item.Visibility = isVisible ? Visibility.Visible : Visibility.Collapsed;

            logger.LogInformation("Set navigation view item visibility");
            return true;
        }
        catch (Exception ex)
        {
        ...
        NavigationViewItem? item)
    {
        ...
        if ($"Views.{item.Content}View".AsType() is null)
            return false;
    ...
    /// <summary>
    /// Navigates to the given navigation view item and clears the backstack
    /// </summary>
    /// <param name="item">The navigation view item which should be set</param>
    /// <returns>A boolean wether the navigation view item has been navigated to successfully</returns>
    public bool NavigateSilent(
        NavigationViewItem? item)
    {
        if (item is null || string.IsNullOrWhiteSpace(item.Content.ToString()))
            return false;
        ...
        clearBackStack = true;
        bool navigate = SetCurrentNavigationViewItem(item);
    ...
    public bool NavigateSilent(
        string page) =>
        NavigateSilent(GetNavigationViewItem(page));
